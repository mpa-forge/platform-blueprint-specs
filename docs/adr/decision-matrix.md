...
| Environment model | `local` + `rc` + `prod`; prod fully separate | Locked | `docs/standards/environment-and-region.md` |
| Primary region | `us-east4` for `rc` and `prod` | Locked | `docs/standards/environment-and-region.md` |
| API runtime path | Cloud Run baseline; GKE Autopilot + Helm alternative | Locked | `ops/api-runtime-paths-cloud-run-gke.md` |
| Initial cluster policy | Do not create initial GKE cluster; enable when needed | Locked | `ops/api-runtime-paths-cloud-run-gke.md`, `platform-specification.md` |
| Repo strategy | Polyrepo with dedicated repos per frontend/api/worker/contracts/infra/ai-workers | Locked | `platform-specification.md` (Section 10), `docs/governance/repo-ownership.md` |
| Auth provider | Clerk Free, B2C-first | Locked | `platform-specification.md` (Section 10), `docs/governance/provider-account-inventory.md` (`P0-T03C`) |
...
| Secrets model | GSM direct on Cloud Run; GSM + ESO on GKE | Locked | `platform-specification.md` (Sections 4, 10), `docs/security/access-model.md` |
...
| CD operating model | Pipeline-driven deploy to Cloud Run baseline; Helm for GKE path | Locked | `platform-specification.md` (Section 10), `implementation-plan.md` |
...
| AI task-to-code runtime | Dedicated `platform-ai-workers` with local/cloud parity, Cloud Run Jobs wake-ups | Locked | `platform-specification.md` (Sections 10, 17), `ops/ai-worker-local-cloud-parity.md` |
...
| Security/access model | Least-privilege roles, WIF for CI, environment-scoped secrets | Locked | `docs/security/access-model.md` |
