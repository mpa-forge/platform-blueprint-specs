...
| RC cloud resources created from clean state | Phase 5 infra apply (`P5-T01`..`P5-T11`) | The required `rc` cloud foundation exists in a reproducible way: network, Cloud Run service base, Cloud SQL, GAR, GSM, `/api/*` routing resources, dashboard provisioning, and AI-worker runtime prerequisites. | `Not started` | Most of Phase 5, especially environment stacks (`P5-T09`), edge routing (`P5-T15`), and clean-state apply validation (`P5-T11`). |
...
| Frontend, auth, API, DB, and routing all connected in `rc` | RC E2E deployment validation (`P6-T06`, `P6-T08`, `P6-T14`, `P6-T10`) | The main cloud objective is achieved: CDN-served frontend uses Clerk auth and same-domain `/api/*` routing to reach the deployed API, which reads from the cloud DB in `rc`. | `Not started` | Frontend CDN path, same-domain routing, frontend publish pipeline, and formal `rc` validation report. |
...
- The main gap between "works locally" and "works in cloud" is Phase 5 plus the Cloud Run/CDN routing work in Phase 6.
